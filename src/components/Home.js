...
function Home() {
  ...
  // Table sorting state (separate from dashboard data)
  const [tableSortConfig, setTableSortConfig] = useState({ key: 'DateObj', direction: 'desc' });
  const [tableData, setTableData] = useState([]);

  // Constants (same as in the original HTML)
  ...
  // Utility functions (converted from the original script)
  ...
  const loadDashboardData = async (plantValue) => {
    ...
    try {
      ...
      // Set headers for the full table
      const headers = ['Date', ...scrapKeys];
      setCurrentHeaders(headers);

      // Check if selectedScrap is still valid
  ...
  useEffect(() => {
    ...
    setTableData(filtered); // Initialize table data with filtered data
  ...
  // Update chart when selected scrap or data changes
  useEffect(() => {
  ...
  // Update table data when filtered data or sort config changes
  useEffect(() => {
    ...
    const sorted = [...filteredData].sort((a, b) => {
      ...
      if (tableSortConfig.key === 'DateObj') {
        aVal = a.DateObj ? a.DateObj.getTime() : Infinity; // Invalid dates to end
        bVal = b.DateObj ? b.DateObj.getTime() : Infinity; // Invalid dates to end
      } else if (tableSortConfig.key === 'Date') {
        // Sort by string representation if DateObj is invalid
  ...
  return (
    <div className="bg-gray-900 text-gray-100 min-h-screen p-4 md:p-8 antialiased">
      ...
      <header className="flex flex-col md:flex-row justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-white mb-4 md:mb-0">Scrap Dashboard</h1>
        <p className="text-gray-400 text-sm">{currentDateTime || 'Loading date...'}</p>
      </header>
            ...
            <div className="flex items-center space-x-2 bg-gray-800 p-1 rounded-lg">
              {['both', 'rml1', 'rml6'].map(plant => (
                <button
                  ...
                  className={`plant-btn px-4 py-2 text-sm font-medium rounded-md transition-colors duration-200 text-gray-200 hover:bg-gray-700 ${
                    currentPlant === plant ? 'btn-active' : ''
                  }`}
                  aria-pressed={currentPlant === plant}
                >
                  {getPlantName(plant)}
                </button>
              ))}
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-xl flex items-center space-x-4 border-l-4 border-amber-500">
          ...
          <div>
            <p className="text-sm font-medium text-gray-400">Casting (T)</p>
            <p id="kpi-casting" className="text-3xl font-bold text-white">{kpis.casting}</p>
          </div>
        </div>
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-xl flex items-center space-x-4 border-l-4 border-red-600">
          ...
          <div>
            <p className="text-sm font-medium text-gray-400">HPTM (T)</p>
            <p id="kpi-hptm" className="text-3xl font-bold text-white">{kpis.hptm}</p>
          </div>
        </div>

        {/* CML */}
        <div id="kpi-cml-card" className={`bg-gray-800 p-6 rounded-lg shadow-xl flex items-center space-x-4 border-l-4 border-green-600 transition-opacity ${kpis.cmlHidden ? 'opacity-50' : ''}`}>
          <div className="bg-green-600 p-3 rounded-full">
            <svg className="w-6 h-6 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" role="img" aria-label="gauge star">
          ...
          <div>
            <p className="text-sm font-medium text-gray-400">CML (T)</p>
            <p id="kpi-cml" className="text-3xl font-bold text-white">{kpis.cml}</p>
          </div>
        </div>
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-xl flex items-center space-x-4 border-l-4 border-indigo-600">
          <div className="bg-indigo-600 p-3 rounded-full">
            <svg className="w-6 h-6 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" role="img" aria-label="forklift icon">
              ...
              <path d="M503.467 119.467h-76.8c-4.719 0-8.533 3.814-8.533 8.533v42.667H512V128c0-4.719-3.814-8.533-8.533-8.533z"/>
            </svg>
          </div>
          <div>
            <p className="text-sm font-medium text-gray-400">Handling (T)</p>
            <p id="kpi-handling" className="text-3xl font-bold text-white">{kpis.handling}</p>
          </div>
        </div>
      </section>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-1/3 bg-gray-800 rounded-lg shadow-lg overflow-hidden">
          <div className="p-4 border-b border-gray-700">
            <h2 id="scrap-list-title" className="text-lg font-semibold text-white">Scrap Details ({getPlantName(currentPlant)})</h2>
          </div>
          <div className="flex text-xs text-gray-400 font-medium px-4 py-2 bg-gray-900">
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            {scrapList.length === 0 ? (
              ...
              scrapList.map(item => (
                <div
                  key={item.key}
                  className={`flex justify-between items-center p-4 cursor-pointer border-b border-gray-700 ${
                    item.isActive ? 'bg-gray-700' : 'hover:bg-gray-700'
                  }`}
                  onClick={() => handleScrapSelect(item.key)}
                >
                  <div className="w-3/6 flex items-center gap-3">
                    <span className="font-semibold text-sm text-white">{item.key}</span>
                  </div>
                  <div className="w-2/6 text-right">
                    <div className="font-bold text-sm text-white">{formatNum(item.currentQty)}</div>
                    <div className="text-xs text-gray-400">Unit (T)</div>
                  </div>
        ...
        <div className="w-full lg:w-2/3 flex flex-col gap-6">
          ...
          <div className="bg-gray-800 rounded-lg shadow-lg p-4 md:p-6">
            <h2 className="text-xl font-semibold text-white mb-4">{selectedScrap} Stock Trend (T)</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-gray-800 p-5 rounded-lg shadow-lg">
              <h3 className="text-gray-400 text-sm font-medium">Current Stock (T)</h3>
              <p className="text-3xl font-bold mt-2 text-white">{chartSummary.current}</p>
            </div>
            <div className="bg-gray-800 p-5 rounded-lg shadow-lg">
              <h3 className="text-gray-400 text-sm font-medium">Change (vs. First Point)</h3>
      ...
      {/* Full Data Table */}
      <section className="mt-6 bg-gray-800 rounded-lg shadow-lg overflow-hidden">
        <div className="p-4 border-b border-gray-700">
          <h2 className="text-lg font-semibold text-white">Full Data ({getPlantName(currentPlant)})</h2>
        </div>
          ...
          <table className="min-w-full">
            <thead className="bg-gray-700">
              <tr>
                {currentHeaders.map((header, index) => (
                  <th
                    key={header}
                    className="sortable-th text-left text-xs font-semibold text-white uppercase tracking-wider p-3"
                    onClick={() => {
            ...
            <tbody className="bg-gray-800">
              {tableData.map((row, index) => (
                <tr key={index} className="bg-gray-800 hover:bg-gray-700 border-t border-gray-700">
                  {currentHeaders.map(header => (
                    <td key={header} className="p-3 text-sm text-white">
